...
void rx_f(rxData *rx){
    ...
    memcpy(modem->tx.data.buf, rx->buf, 25);//copy data we'll sent to buffer
    modem->tx.data.size = 25;//Payload len
    ...
    printf("payloadSymbNb: %u\n", modem->tx.data.payloadSymbNb);
        ...
        FILE *fp;
        fp = fopen("/home/pi/sensor.txt", "wt");
        fprintf(fp, rx->buf);
        fclose(fp);
...
int main(){
    ...
    modem.eth.resetGpioN = 17;//GPIO4 on lora RESET pin
