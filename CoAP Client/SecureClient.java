...
public class SecureClient {
        ...
        private static final String SERVER_URI = "coaps://127.0.0.1:5684/";
        ...
        public void test() {
                ...
                try {
                        URI uri = new URI(SERVER_URI + "example1");
                        ...
                        final CoapObserveRelation relation = client.observe(new CoapHandler(){
                                ...
                                public void  onLoad(CoapResponse response){
                                        ...
                                        //System.out.println("changed of value: " + content);
                                        System.out.println(response.getCode() + " - " + response.getCode().name());
                                        System.out.println(response.getOptions());
                                        System.out.println(response.getResponseText());
                                        InsertDB.insert(response.getResponseText());
                                        InsertDB.update(response.getResponseText());
                                        System.out.println();
                                        System.out.println("ADVANCED:");
                                        EndpointContext context = response.advanced().getSourceContext();
                                        Principal identity = context.getPeerIdentity();
                                        if (identity != null) { System.out.println(context.getPeerIdentity());}
     ...
     // this part is executed when an exception (in this example InterruptedException) occur
                 //catch (IOException e) {
